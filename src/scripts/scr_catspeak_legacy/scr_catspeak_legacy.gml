...
#macro __CATSPEAK_LEGACY_UNIMPLEMENTED show_error("unimplemented Catspeak function", true)

/// @desc The container that manages the Catspeak compiler processes.
function __catspeak_legacy_manager() {
    static catspeak = {
        sessions : [],
        sessionEmpty : [],
        compilerProcesses : [],
...
function __catspeak_legacy_handle_error(_e) {
    var catspeak = __catspeak_legacy_manager();
...
function __catspeak_legacy_kill_compiler_process() {
    var catspeak = __catspeak_legacy_manager();
...
function __catspeak_legacy_kill_runtime_process(_process_id) {
    var catspeak = __catspeak_legacy_manager();
    array_delete(catspeak.runtimeProcesses, _process_id, 1);
    catspeak.runtimeProcessCount -= 1;
    __catspeak_legacy_next_runtime_process();
}

/// @desc Moves onto the next runtime process.
function __catspeak_legacy_next_runtime_process() {
    var catspeak = __catspeak_legacy_manager();
...
function catspeak_legacy_start_frame() {
    var catspeak = __catspeak_legacy_manager();
    catspeak.frameStartTime = get_timer();
}

/// @desc Updates the catspeak manager.
function catspeak_legacy_update() {
    var catspeak = __catspeak_legacy_manager();
    ...
    do {
        ...
        if (compiler_process != undefined) {
            ...
            if not (compiler.inProgress()) {
                ...
                __catspeak_legacy_kill_compiler_process();
                continue;
            }
            try {
                ...
                __catspeak_legacy_kill_compiler_process();
                __catspeak_legacy_handle_error(_e);
            }
            ...
            if not (runtime.inProgress()) {
                __catspeak_legacy_kill_runtime_process(process_id);
                continue;
            }
            try {
...
function catspeak_legacy_set_error_script(_f) {
    var catspeak = __catspeak_legacy_manager();
...
function catspeak_legacy_set_max_iterations(_iteration_count) {
    var catspeak = __catspeak_legacy_manager();
...
function catspeak_legacy_set_frame_allocation(_amount) {
    var catspeak = __catspeak_legacy_manager();
...
function catspeak_legacy_session_create() {
    ...
    var f = catspeak_legacy_session_add_function;
    var c = catspeak_legacy_session_add_constant;
...
function catspeak_legacy_session_destroy(_session_id) {
    var catspeak = __catspeak_legacy_manager();
    var sessions = catspeak.sessions;
    var empty_sessions = catspeak.sessionEmpty;
...
function catspeak_legacy_session_set_source(_session_id, _src) {
    var catspeak = __catspeak_legacy_manager();
...
function catspeak_legacy_session_enable_global_access(_session_id, _enable) {
    var sessions = __catspeak_legacy_manager().sessions;
...
/// @param {struct} session_id The ID of the session to update.
/// @param {bool} enable Whether to enable this option.
function catspeak_legacy_session_enable_instance_access(_session_id, _enable) {
    var sessions = __catspeak_legacy_manager().sessions;
    var session = sessions[@ _session_id];
...
/// @param {struct} session_id The ID of the session to update.
/// @param {bool} enable Whether to enable this option.
function catspeak_legacy_session_enable_implicit_return(_session_id, _enable) {
    var sessions = __catspeak_legacy_manager().sessions;
    var session = sessions[@ _session_id];
...
/// @param {struct} session_id The ID of the session to update.
/// @param {bool} enable Whether to enable this option.
function catspeak_legacy_session_enable_shared_workspace(_session_id, _enable) {
    var sessions = __catspeak_legacy_manager().sessions;
    var session = sessions[@ _session_id];
...
function catspeak_legacy_session_add_constant(_session_id, _name, _value) {
    var catspeak = __catspeak_legacy_manager();
    var session = catspeak.sessions[@ _session_id];
...
function catspeak_legacy_session_add_function(_session_id, _name, _value) {
    var catspeak = __catspeak_legacy_manager();
    var session = catspeak.sessions[@ _session_id];
    var f = _value;
    if not (is_method(f)) {
...
function catspeak_legacy_session_create_process(_session_id, _callback_return, _args=[]) {
    var catspeak = __catspeak_legacy_manager();
    var session = catspeak.sessions[@ _session_id];
    var chunk = session.compilerProcess == undefined ? __catspeak_legacy_default_chunk() : session.compilerProcess.chunk;
...
/// @param {real} session_id The ID of the session to spawn a process for.
/// @param {array} args The arguments to pass to the process.
function catspeak_legacy_session_create_process_greedy(_session_id, _args=[]) {
    var catspeak = __catspeak_legacy_manager();
    var session = catspeak.sessions[@ _session_id];
    var chunk;
    var compiler_process = session.compilerProcess;
    if (compiler_process == undefined) {
        chunk = __catspeak_legacy_default_chunk();
    } else {
...
function __catspeak_legacy_get_unordered_collection_keys(_container) {
...
/// @desc Attempts to get the length of an ordered container.
/// @param {value} container The container to index.
function __catspeak_legacy_get_ordered_collection_length(_container) {
...
function __catspeak_legacy_identifier_is_valid_hole(_name) {
...
function __catspeak_legacy_encode_value(_value) {
    if (is_string(_value)) {
        return _value;
        ...
        for (var i = 0; i < n; i += 1) {
            ...
            inner += __catspeak_legacy_encode_value(_value[i]);
        }
        return "[" + inner + "]";
        ...
        for (var i = array_length(names) - 1; i >= 0; i -= 1) {
            var name = names[i];
            if (__catspeak_legacy_identifier_is_valid_hole(name)) {
                continue;
            }
            if (count != 0) {
                inner += ", ";
            ...
            count += 1;
            inner += name + " : " + __catspeak_legacy_encode_value(_value[$ name]);
        }
        return "{" + inner + " }";
    } else {
...
function __catspeak_legacy_token_render(_kind) {
    switch (_kind) {
...
function __catspeak_legacy_token_is_operator(_token) {
...
function __catspeak_legacy_byte_is_newline(_byte) {
    switch (_byte) {
    case ord("\n"):
    case ord("\r"):
        return true;
...
/// @param {real} byte The byte to check.
function __catspeak_legacy_byte_is_not_newline(_byte) {
    return !__catspeak_legacy_byte_is_newline(_byte);
}

/// @desc Returns whether a byte is a valid quote character.
/// @param {real} byte The byte to check.
function __catspeak_legacy_byte_is_quote(_byte) {
    return _byte == ord("\"");
}

/// @desc Returns whether a byte is NOT a valid quote character.
/// @param {real} byte The byte to check.
function __catspeak_legacy_byte_is_not_quote(_byte) {
    return !__catspeak_legacy_byte_is_quote(_byte);
}

/// @desc Returns whether a byte is a valid quote character.
/// @param {real} byte The byte to check.
function __catspeak_legacy_byte_is_accent(_byte) {
    return _byte == ord("`");
}

/// @desc Returns whether a byte is NOT a valid quote character.
/// @param {real} byte The byte to check.
function __catspeak_legacy_byte_is_not_accent(_byte) {
    return !__catspeak_legacy_byte_is_accent(_byte);
}
...
function __CatspeakScanner(_buff) constructor {
    ...
    static next = function() {
        ...
        case ord("\\"):
            ...
            advanceWhile(__catspeak_legacy_byte_is_newline);
            advanceWhile(__catspeak_legacy_byte_is_whitespace);
        ...
        case ord("^"):
        case ord("$"):
            advanceWhile(__catspeak_legacy_byte_is_operator);
            registerLexeme();
        ...
        case ord("-"):
            ...
            if (isCommentLexeme && lexemeLength > 1) {
                advanceWhile(__catspeak_legacy_byte_is_not_newline);
        ...
        case ord("\""):
            clearLexeme();
            advanceWhile(__catspeak_legacy_byte_is_not_quote);
            skipNextByte = true;
            registerLexeme();
        ...
        case ord("`"):
            clearLexeme();
            advanceWhile(__catspeak_legacy_byte_is_not_accent);
            skipNextByte = true;
            registerLexeme();
            return __CatspeakToken.IDENTIFIER;
        default:
            if (__catspeak_legacy_byte_is_newline(byte)) {
                advanceWhile(__catspeak_legacy_byte_is_newline);
                return __CatspeakToken.EOL;
            } else if (__catspeak_legacy_byte_is_whitespace(byte)) {
                advanceWhile(__catspeak_legacy_byte_is_whitespace);
                return __CatspeakToken.WHITESPACE;
            } else if (__catspeak_legacy_byte_is_alphabetic(byte)) {
                advanceWhile(__catspeak_legacy_byte_is_alphanumeric);
                registerLexeme();
                var keyword;
                switch (lexeme) {
                case "if":
                ...
                lexeme = undefined;
                return keyword;
            } else if (__catspeak_legacy_byte_is_digit(byte)) {
                if (byte == ord("0")) {
                    switch (peek(1)) {
                    case ord("x"):
                        if not (__catspeak_legacy_byte_is_hex_digit(peek(2))) {
                            break;
                        }
                        advance();
                        clearLexeme();
                        advanceWhile(__catspeak_legacy_byte_is_hex_digit);
                        registerLexeme();
                        return __CatspeakToken.NUMBER_HEX;
                    case ord("b"):
                        if not (__catspeak_legacy_byte_is_bin_digit(peek(2))) {
                            break;
                        }
                        advance();
                        clearLexeme();
                        advanceWhile(__catspeak_legacy_byte_is_bin_digit);
                        registerLexeme();
                        return __CatspeakToken.NUMBER_BIN;
                    }
                }
                advanceWhile(__catspeak_legacy_byte_is_digit);
                if (peek(1) == ord(".") && __catspeak_legacy_byte_is_digit(peek(2))) {
                    advance();
                    advanceWhile(__catspeak_legacy_byte_is_digit);
                    registerLexeme();
        ...
        while (true) {
            ...
            case __CatspeakToken.EOL:
                ...
                case __CatspeakToken.ADDITION:
                    ...
                    break;
                default:
                    if (__catspeak_legacy_token_is_operator(pred)) {
                        implicit_semicolon = false;
                    }
                }
                switch (succ) {
                ...
                case __CatspeakToken.ADDITION:
                    ...
                    break;
                default:
                    if (__catspeak_legacy_token_is_operator(succ)) {
                        implicit_semicolon = false;
                    }
                }
...
function __catspeak_legacy_compiler_state_render(_state) {
    switch (_state) {
...
function __CatspeakCompiler(_lexer, _out) constructor {
    ...
    static matchesOperator = function() {
        return __catspeak_legacy_token_is_operator(peeked);
    }
    ...
    static errorAndAdvance = function(_msg) {
        advance();
        error(_msg + " -- got `" + string(lexeme) + "` (" + __catspeak_legacy_token_render(token) + ")");
    }
    ...
    static generateCode = function() {
        ...
        case __CatspeakCompilerState.CALL_END:
            var arg_count = popStorage();
            out.addCode(pos, __CatspeakOpCode.CALLSPAN, arg_count);
            break;
        ...
        case __CatspeakCompilerState.OBJECT:
            ...
            break;
        default:
            error("unknown compiler instruction `" + string(state) + "` (" + __catspeak_legacy_compiler_state_render(state) + ")");
            break;
        }
        if not (inProgress()) {
...
function __catspeak_legacy_code_render(_kind) {
    switch (_kind) {
    ...
    case __CatspeakOpCode.CALLSPAN: return "CALLSPAN";
...
function __CatspeakChunk() constructor {
    ...
    static toString = function() {
        ...
        for (var i = 0; i < size; i += 1) {
            ...
            out += string(i) + " " + __catspeak_legacy_code_render(inst.code);
...
function __catspeak_legacy_default_chunk() {
    static init = true;
...
function __CatspeakVM(_chunk, _max_iterations, _global_access, _instance_access,
    ...
    static popManyKWArgs = function(_n) {
        ...
        repeat (_n) {
            var value = pop();
            var key = pop();
            if (__catspeak_legacy_identifier_is_valid_hole(key)) {
                continue;
            }
            values[$ __catspeak_legacy_encode_value(key)] = value;
        }
        return values;
    }
    ...
    static setVariable = function(_name, _value) {
        if (__catspeak_legacy_identifier_is_valid_hole(_name)) {
            return;
        }
    ...
    static getIndex = function(_container, _subscript, _unordered) {
        ...
        if (_unordered) {
            switch (ty) {
            case "struct":
                return __catspeak_legacy_identifier_is_valid_hole(_subscript) ?
                        undefined : _container[$ __catspeak_legacy_encode_value(_subscript)];
            case "number":
            case "bool":
            case "int32":
            case "int64":
    ...
    static setIndex = function(_container, _subscript, _unordered, _value) {
        ...
        if (_unordered) {
            switch (ty) {
            case "struct":
                if not (__catspeak_legacy_identifier_is_valid_hole(_subscript)) {
                    _container[$ __catspeak_legacy_encode_value(_subscript)] = _value;
                }
                return;
            case "number":
            case "bool":
    ...
    static computeProgram = function() {
        ...
        case __CatspeakOpCode.MAKE_ITERATOR:
            ...
            iterators[$ options.idx] = {
                ...
                value : options.value,
                subscript : 0,
                indices : unordered ? __catspeak_legacy_get_unordered_collection_keys(container) : undefined,
            };
            break;
        case __CatspeakOpCode.UPDATE_ITERATOR:
            ...
            if (unordered) {
                ...
                if (subscript >= __catspeak_legacy_get_ordered_collection_length(container)) {
                    push(false);
                    break;
                }
            }
        ...
        case __CatspeakOpCode.PRINT:
            var value = pop();
            show_debug_message(__catspeak_legacy_encode_value(value));
            break;
        ...
        case __CatspeakOpCode.JUMP_FALSE:
            ...
            break;
        case __CatspeakOpCode.CALLSPAN:
        ...
        case __CatspeakOpCode.RETURN:
            ...
            return;
        default:
            error("unknown program instruction `" + string(inst.code) + "` (" + __catspeak_legacy_code_render(inst.code) + ")");
            break;
        }
        pc += 1;
    }
