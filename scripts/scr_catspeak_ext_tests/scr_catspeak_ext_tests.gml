...
try {
    // basic functionality
    var session = catspeak_session_id_create();
    var this = { result : undefined };
    catspeak_session_id_add_result_handler(session, method(this, function(_result) {
        result = _result;
    }));
    catspeak_session_id_add_source(session, "return 12");
    catspeak_session_id_update_eager(session);
    catspeak_session_id_destroy(session);
    __catspeak_ext_tests_assert_eq(12, this.result);

    // empty source code and comments
    var session = catspeak_session_id_create();
    var this = { error : false };
    catspeak_session_id_add_error_handler(session, method(this, function(_) {
        error = true;
    }));
    catspeak_session_id_add_source(session, "");
    catspeak_session_id_add_source(session, "\n\n\t");
    catspeak_session_id_add_source(session, @'-- hello
    --another comment');
    catspeak_session_id_add_source(session, "\r\n\n");
    catspeak_session_id_update_eager(session);
    catspeak_session_id_destroy(session);
    ...
    catspeak_session_id_add_expression_statement_handler(session, method(this, function(_value) {
        value = _value;
    }));
    catspeak_session_id_add_source(session, "18");
    catspeak_session_id_update_eager(session);
    catspeak_session_id_destroy(session);
    ...
    var session = catspeak_session_id_create();
    var workspace = catspeak_session_id_get_workspace(session);
    catspeak_session_id_add_source(session, "set var 3; set var' var");
    catspeak_session_id_update_eager(session);
    catspeak_session_id_destroy(session);
    ...
    var session = catspeak_session_id_create();
    var workspace = catspeak_session_id_get_workspace(session);
    catspeak_session_id_add_constant(session, "const", -12);
    catspeak_session_id_add_function(session, "funct", max);
    catspeak_session_id_add_source(session, "set const' const; set funct' funct");
    catspeak_session_id_update_eager(session);
    catspeak_session_id_destroy(session);
    ...
    var session = catspeak_session_id_create();
    var workspace = catspeak_session_id_get_workspace(session);
    catspeak_ext_session_id_add_gml_operators(session);
    catspeak_ext_session_id_add_gml_maths(session);
    catspeak_session_id_add_source(session, @'
    set a : -3
    set b : 12
    set c : a * b + -a
    ...
    var workspace = catspeak_session_id_get_workspace(session);
    catspeak_ext_session_id_add_gml_operators(session);
    catspeak_session_id_add_function(session, "register_count", method(this, function() {
        count += 1;
    }));
    catspeak_session_id_add_source(session, @'
    set count 0
    ...
    var workspace = catspeak_session_id_get_workspace(session);
    catspeak_ext_session_id_add_gml_maths(session);
    catspeak_session_id_add_function(session, "failure", function(_number) {
